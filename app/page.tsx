...
const categoryConfig: Record<string, { useGod: string[]; focus: string; risk: string }> = {
  求财: { useGod: ["妻财", "子孙"], focus: "财爻与世爻承接", risk: "兄弟旺、财弱、动而受克" },
  感情: { useGod: ["官鬼", "妻财"], focus: "世应关系与对方星", risk: "世应冲克、用神空破" },
  考试: { useGod: ["父母", "官鬼"], focus: "父母爻、官鬼爻、世爻状态", risk: "父母弱、世弱、临冲破" },
  工作: { useGod: ["官鬼", "父母"], focus: "职位、流程、沟通承接", risk: "官鬼空破、应不接世" },
  综合: { useGod: ["世爻"], focus: "先看世应和整体格局", risk: "单点信号过强导致误判" },
};

function detectCategory(text: string) {
  const t = text.trim();
  if (!t) return "综合";
  if (/(财|收入|钱|投资|奖金|生意|求财|财运)/.test(t)) return "求财";
...
function seededLine(rand: () => number): LineValue {
  ...
  return tosses.reduce((a, b) => a + b, 0) as LineValue;
}

function relation(selfElement: string, otherElement: string) {
...
function generatedBy(el: string) {
  return Object.keys(generates).find((key) => generates[key] === el) || "";
}
...
function formatShortDate(dt: Date | null) {
  if (!dt) return "";
...
function buildReading(question: string, dt: Date, lines: number[]) {
  ...
  const changedItems = buildLines(getChangedLines(lines), dt).map((item, idx) => ({
    ...
    strength: scoreLineStrength(item, monthBranch, dayBranch),
    transform:
      items[idx].element === item.element ? "回头平" : generatedBy(items[idx].element) === item.element ? "回头生" : "回头克",
  }));
  ...
  return {
    ...
    useGodText: useGod
      ? `重点参考位在第 ${useGod.line} 爻 ${useGod.relation}，旺衰 ${useGod.strength.level}。`
      : "当前用神不集中，需结合整体结构判断。",
    worldText:
      world && response
        ? `世爻在第 ${world.line} 爻，旺衰 ${world.strength.level}；应爻在第 ${response.line} 爻，旺衰 ${response.strength.level}。`
        : "世应信息不足，当前以整体卦势判断。",
...
function LineRow({ item, changed }: { item: LineMeta; changed: LineMeta & { transform: string } }) {
  return (
    <div
      style={{
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(5, minmax(0, 1fr))", gap: 8, fontSize: 14 }}>
        ...
        <div>
          ...
          <div style={{ marginTop: 4 }}>
            {item.relation} / {item.element}
          </div>
        </div>
        <div>
          <b>地支</b>
          <div style={{ marginTop: 4 }}>
            {item.branch} / {item.god}
          </div>
        </div>
        <div>
          <b>变化</b>
          <div style={{ marginTop: 4 }}>
            {item.moving ? "动" : "静"} → {changed.relation}
          </div>
        </div>
      </div>
...
export default function Page() {
  ...
  const [tab, setTab] = useState<"overview" | "timing" | "details" | "analysis">("overview");
        ...
        >
          <div style={{ display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap", alignItems: "flex-start" }}>
            <div>
              ...
              <div style={{ marginTop: 14, fontSize: 38, fontWeight: 900, letterSpacing: "-0.04em", color: "#0f172a" }}>
                六爻
              </div>
            </div>
          ...
          <div style={{ background: "#ffffff", border: "1px solid #e4ebf3", borderRadius: 28, padding: 22, boxShadow: "0 10px 26px rgba(15,23,42,0.03)" }}>
            ...
            <div style={{ marginTop: 18, display: "grid", gap: 10 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 10 }}>
                ...
                <button
                  onClick={() => reading && exportText(reading, question)}
                  disabled={!reading}
                  style={{
                    padding: "12px 14px",
                    borderRadius: 16,
                    border: "1px solid #dde7f0",
                    background: reading ? "#fff" : "#f8fafc",
                    color: reading ? "#111827" : "#9ca3af",
                ...
                <button
                  onClick={() => reading && printReport(reading, question)}
                  disabled={!reading}
                  style={{
                    padding: "12px 14px",
                    borderRadius: 16,
          ...
          <>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {[
                ...
                <button
                  key={key}
                  onClick={() => setTab(key as "overview" | "timing" | "details" | "analysis")}
                  style={{
                    padding: "11px 16px",
                    borderRadius: 999,
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1.65fr 1fr", gap: 18 }}>
                <div style={{ background: "#ffffff", border: "1px solid #e4ebf3", borderRadius: 28, padding: 24, boxShadow: "0 10px 26px rgba(15,23,42,0.03)" }}>
                  ...
                  <div style={{ marginTop: 18, padding: 18, borderRadius: 24, border: `1px solid ${tone.bd}`, background: tone.bg }}>
                    ...
                    <div style={{ marginTop: 8, fontSize: 32, fontWeight: 900, letterSpacing: "-0.03em", color: tone.tx }}>{reading.action}</div>
                  </div>
                ...
                <div style={{ background: "#ffffff", border: "1px solid #e4ebf3", borderRadius: 28, padding: 24 }}>
                  ...
                  <div style={{ marginTop: 18, display: "grid", gap: 14 }}>
                    ...
                    <div style={{ padding: 18, borderRadius: 20, background: "#f8fafc", border: "1px solid #e5edf5", lineHeight: 1.8 }}>
                      盘面信号：{reading.interactionNotes.length ? reading.interactionNotes.join("、") : "当前未见明显合冲主导，仍以旺衰和世应为主。"}
                    </div>
                  </div>
                </div>
