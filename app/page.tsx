...
function CardTitle({ icon, title, sub }: { icon: React.ReactNode; title: string; sub?: string }) {
  return (
    <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
      <div style={{ width: 40, height: 40, borderRadius: 12, background: "#f4f7fb", border: "1px solid #e5edf5", display: "flex", alignItems: "center", justifyContent: "center" }}>
        {icon}
      </div>
      <div>
            ...
            <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
              {[
                ...
                <button
                  key={key}
                  onClick={() => setTab(key)}
                  style={{ padding: "11px 16px", borderRadius: 999, border: tab === key ? "1px solid #0f172a" : "1px solid #dbe4ee", background: tab === key ? "#0f172a" : "#ffffff", color: tab === key ? "#ffffff" : "#111827", cursor: "pointer", fontWeight: 600 }}
                >
                  {label}
                </button>
              ))}
